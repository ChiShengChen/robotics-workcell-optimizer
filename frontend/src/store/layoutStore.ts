...
export const useLayoutStore = create<LayoutState>()(
  persist(
    (set, get) => ({
      ...
      loadCadSample: async (id) => {
        set({ errors: [] })
        try {
          const r = await api.loadCadSample(id)
          const existing = get().spec
          const envelope = r.suggested_cell_envelope_mm ?? existing?.cell_envelope_mm ?? [8000, 6000]
          const newSpec: WorkcellSpec = existing
            ? {
                ...
                assumptions: [
                  ...existing.assumptions,
                  `Loaded sample CAD '${id}': ${r.n_entities_imported} obstacles; cell envelope set to ${envelope[0].toFixed(0)}×${envelope[1].toFixed(0)} mm.`,
                ],
              }
            : {
                schema_version: '1.0',
                ...
                obstacles: r.obstacles,
                assumptions: [
                  `Loaded sample CAD '${id}' (${r.n_entities_imported} obstacles); enter a prompt + run Extract to fill in the rest of the spec.`,
                ],
                notes: '',
              }
          set({
